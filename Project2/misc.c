...
int tlookup(void)
{
    char mesg[MAX_TOPICS_SIZE];
    
    MessagingErrorCodes mec = lookup_topics(mesg);

    if(mec == 0){
        sys_datacopy(PM_PROC_NR, (vir_bytes) (mesg), m_in.m_source, (vir_bytes) m_in.m1_p1, MAX_TOPICS_SIZE);
    }
    return mec;
}
...
int tcreate(void)
{
    ...
    sys_datacopy(m_in.m_source, (vir_bytes) m_in.m1_p1, PM_PROC_NR, (vir_bytes)topic_name, MAX_TOPIC_NAME_LEN);
    
    MessagingErrorCodes mec = add_topic(topic_name);
...
int tpublisher(void)
{
    ...
    MessagingErrorCodes mec = add_publisher_to_topic(topic_name, pubId);

    return mec;
}

int tsubscriber(void)
{
    char topic_name[MAX_TOPIC_NAME_LEN];
    sys_datacopy(m_in.m_source, (vir_bytes) m_in.m1_p1, PM_PROC_NR, (vir_bytes)topic_name, MAX_TOPIC_NAME_LEN);
    ...
    MessagingErrorCodes mec = add_subscriber_to_topic(topic_name, subId);
...
int tpublish(void)
{
    ...
    sys_datacopy(m_in.m_source, (vir_bytes) m_in.m1_p2, PM_PROC_NR, (vir_bytes)mesg, MAX_MESG_LEN);

    MessagingErrorCodes mec = publish_message(topic_name, mesg);
...
int tretrieve(void)
{
    ...
    MessagingErrorCodes mec = get_next_message(topic_name, mesg, subId);

    if(mec == 0){
        sys_datacopy(PM_PROC_NR, (vir_bytes) (*mesg), m_in.m_source,(vir_bytes) m_in.m1_p2, MAX_MESG_LEN);
    }
    return mec;
